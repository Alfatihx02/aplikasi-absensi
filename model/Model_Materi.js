...
class Model_Materi {
    ...
    static async getByIdMahasiswa(id_mahasiswa){
        return new Promise((resolve, reject) => {
            connection.query(`SELECT m.id_materi, m.file_materi, m.keterangan, mk.nama_matakuliah, j.id_jadwal, 
                j.hari, j.waktu_mulai, j.waktu_selesai FROM materi m JOIN jadwal j ON m.id_jadwal = j.id_jadwal 
                JOIN matakuliah mk ON j.id_matakuliah = mk.id_matakuliah JOIN kelas k ON j.id_kelas = k.id_kelas 
                JOIN mahasiswa mhs ON k.id_kelas = mhs.id_kelas WHERE mhs.id_mahasiswa = ? ORDER BY j.id_jadwal
